...
const MainApp: React.FC = () => {
  ...
  // -- REGULAR ROLES FIX INJECTION v2 --
  useEffect(() => {
    if (localStorage.getItem('users_fixed_geel_v5')) return;
    if (data.users && data.users.length > 0 && updateData && data.schoolBranches) {
      let needsUpdate = false;
      const updatedUsers = data.users.map(u => {
        let newU = { ...u };
        if (newU.role !== 'admin' && newU.role !== 'user') {
          newU.role = 'user';
          newU.expiryDate = newU.expiryDate || '2025-12-31T23:59:59.999Z';
          needsUpdate = true;
        }
        return newU;
      });

      // Collect all newly encountered branches
      const schoolName = "مدارس جيل الرسالة الحديثة";
      const currentBranches = data.schoolBranches[schoolName] || [];
      const newBranches = new Set(currentBranches);
      updatedUsers.forEach(u => {
         const ub = u.permissions?.schoolsAndBranches?.[schoolName] || [];
         ub.forEach(b => newBranches.add(b));
      });
      const newBranchesArr = Array.from(newBranches);

      let branchesUpdate = {};
      if (newBranchesArr.length !== currentBranches.length) {
         branchesUpdate = {
           schoolBranches: {
             ...(data.schoolBranches || {}),
             [schoolName]: newBranchesArr
           }
         };
         needsUpdate = true;
      }

      const currentAvailableSchools = data.availableSchools || [];
      let schoolsUpdate = {};
      if (!currentAvailableSchools.includes(schoolName)) {
         schoolsUpdate = { availableSchools: [...currentAvailableSchools, schoolName] };
         needsUpdate = true;
      }

      if (needsUpdate) {
        console.log("Fixing user roles & branches...");
        updateData({ users: updatedUsers, ...branchesUpdate, ...schoolsUpdate });
        localStorage.setItem('users_fixed_geel_v5', 'true');
      } else {
        localStorage.setItem('users_fixed_geel_v5', 'true');
      }
    }
  }, [data.users, updateData, data.schoolBranches, data.availableSchools]);
  // -- END REGULAR ROLES FIX v2 --
