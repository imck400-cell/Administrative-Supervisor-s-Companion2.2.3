...
export const GradeSheetsView = ({ onBack }: { onBack: () => void }) => {
  ...
  const [schoolName, setSchoolName] = useState(
    currentUser?.selectedSchool || availableSchools[0] || "",
  );
  ...
  const availableBranches = useMemo(() => {
    ...
    const userPerms = data.users?.find((u) => u.id === currentUser?.id)
      ?.permissions?.schoolsAndBranches;
    ...
    return [
      {
        ...
        name: currentUser?.name || "مستخدم غير معروف",
      },
    ];
  }, [currentUser]);
  ...
  const canEditData =
    ...
    (Array.isArray(currentUser?.permissions?.gradeSheets) &&
      currentUser.permissions.gradeSheets.includes("edit_data"));
  ...
  const displayedStudents = useMemo(() => {
    ...
    const studentsWithScore = students.map((s) => {
      ...
      const targetMonths =
        indicatorConfig.months.length > 0
          ? indicatorConfig.months
          : visibleMonths;
      const targetFields =
        indicatorConfig.fields.length > 0
          ? indicatorConfig.fields
          : ["w_k", "w_m", "oral", "attend", "written"];
      ...
      const percentage =
        indicatorMax > 0 ? (indicatorTotal / indicatorMax) * 100 : 0;
    ...
    if (indicatorConfig.level === "highest") {
      return studentsWithScore
        .sort((a, b) => b.indicatorTotal - a.indicatorTotal)
        .slice(0, Math.max(1, Math.floor(studentsWithScore.length / 4)));
    }
    if (indicatorConfig.level === "lowest") {
      return studentsWithScore
        .sort((a, b) => a.indicatorTotal - b.indicatorTotal)
        .slice(0, Math.max(1, Math.floor(studentsWithScore.length / 4)));
    }
    if (indicatorConfig.level === "average") {
      return studentsWithScore
        .sort((a, b) => b.indicatorTotal - a.indicatorTotal)
        .slice(
          Math.floor(studentsWithScore.length / 4),
          Math.floor((studentsWithScore.length * 3) / 4),
        );
    }
    if (indicatorConfig.level === "absent_written") {
      return studentsWithScore
        .filter((s) => !s.hasWritten)
        .sort((a, b) => (a.name || "").localeCompare(b.name || "", "ar"));
    }
    if (indicatorConfig.level === "no_grades") {
      return studentsWithScore
        .filter((s) => !s.hasAnyGrade)
        .sort((a, b) => (a.name || "").localeCompare(b.name || "", "ar"));
    }
    ...
    if (indicatorConfig.level === "vgood") {
      return studentsWithScore
        .filter((s) => s.percentage >= 80 && s.percentage < 90)
        .sort((a, b) => b.indicatorTotal - a.indicatorTotal);
    }
    ...
    if (indicatorConfig.level === "fair") {
      return studentsWithScore
        .filter((s) => s.percentage >= 50 && s.percentage < 65)
        .sort((a, b) => b.indicatorTotal - a.indicatorTotal);
    }
    if (indicatorConfig.level === "fail") {
      return studentsWithScore
        .filter((s) => s.percentage < 50 && s.hasAnyGrade)
        .sort((a, b) => b.indicatorTotal - a.indicatorTotal);
    }
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "GradeSheets"), (snap) => {
      const list = snap.docs.map(
        (d) => ({ id: d.id, ...d.data() }) as GradeSheet,
      );
      setSheets(
        list.filter(
          (s) =>
            s.userId === (currentUser?.id || "unknown") ||
            currentUser?.role === "admin" ||
            currentUser?.permissions?.all === true,
        ),
      );
      ...
      if (currentSheetId) {
        ...
        if (current) {
          setMaxScores((prev) =>
            JSON.stringify(prev) !== JSON.stringify(current.maxScores)
              ? current.maxScores || prev
              : prev,
          );
          setDisabledCols((prev) =>
            JSON.stringify(prev) !== JSON.stringify(current.disabledCols)
              ? current.disabledCols || []
              : prev,
          );
          ...
          if (!snap.metadata.hasPendingWrites) {
            setCells((prev) =>
              JSON.stringify(prev) !== JSON.stringify(current.cells || {})
                ? current.cells || {}
                : prev,
            );
            setSchoolName((prev) =>
              current.schoolName !== prev ? current.schoolName || "" : prev,
            );
            setBranch((prev) =>
              current.branch !== prev ? current.branch || "" : prev,
            );
            setTeacherName((prev) =>
              current.teacherName !== prev ? current.teacherName || "" : prev,
            );
            setSubject((prev) =>
              current.subject !== prev ? current.subject || "" : prev,
            );
            setSheetName((prev) =>
              current.sheetName !== prev ? current.sheetName || "" : prev,
            );
            setMonths((prev) =>
              JSON.stringify(prev) !== JSON.stringify(current.months || [])
                ? current.months || prev
                : prev,
            );
          }
  ...
  const updateStudentCalculations = (
    currentCells: Record<string, string>,
    studentId: string,
    currentMaxScores: Record<string, number>,
    currentVisibleMonths: number[] = visibleMonths,
  ) => {
    months.forEach((_, idx) => {
      ...
      ["w_k", "w_m", "oral", "attend", "written"].forEach((mc) => {
        monthTotal +=
          parseFloat(currentCells[`${studentId}-m${mIdx}-${mc}`] || "0") || 0;
      });

      currentCells[`${studentId}-m${mIdx}-total_m`] = monthTotal.toString();

      const monthMaxSum =
        (currentMaxScores.w_k || 10) +
        (currentMaxScores.w_m || 10) +
        (currentMaxScores.oral || 10) +
        (currentMaxScores.attend || 10) +
        (currentMaxScores.written || 60);
      const ratio = monthMaxSum > 0 ? (monthTotal / monthMaxSum) * 100 : 0;
      currentCells[`${studentId}-m${mIdx}-ratio_m`] =
    ...
    currentVisibleMonths.forEach((idx) => {
      sumOfMonths +=
        parseFloat(currentCells[`${studentId}-m${idx}-total_m`] || "0") || 0;
    });

    const maxAllMonths = monthMaxSum * currentVisibleMonths.length;
    const termResult =
      maxAllMonths > 0 ? (sumOfMonths / maxAllMonths) * termTargetMax : 0;

    if (sumOfMonths > 0) {
      currentCells[`${studentId}-final-term_result`] = termResult
        .toFixed(1)
        .replace(/\.0$/, "");
  ...
  const handleColToggle = (colKey: string) => {
    ...
    const newDisabled = disabledCols.includes(colKey)
      ? disabledCols.filter((c) => c !== colKey)
      : [...disabledCols, colKey];
  ...
  const handleSave = async (
    currentCells: Record<string, string> = cells,
    currentDisabledCols = disabledCols,
    currentMaxScores = maxScores,
    isAutoSave = false,
  ) => {
    let missingFields = [];
    ...
    if (missingFields.length > 0) {
      if (!isAutoSave)
        toast.error(
          `الرجاء إكمال البيانات التالية قبل الحفظ: ${missingFields.join("، ")}`,
        );
      return;
    }
  ...
  const handleCombine = () => {
    ...
    const sheetsToCombine = sheets.filter((s) =>
      selectedCombineSheets.includes(s.id || ""),
    );
    ...
    <div className="max-w-[1400px] mx-auto p-4 md:p-8 font-arabic animate-fade-in relative z-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-2">
            إعداد وإدارة كشوف المتابعة والدرجات
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 md:gap-3 text-sm md:text-base">
          <button
            onClick={() => setShowHistoryModal(true)}
            className="px-3 py-2 md:px-4 md:py-2 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-xl transition-colors font-bold flex items-center gap-1 md:gap-2 shadow-sm whitespace-nowrap"
          >
            <History size={18} /> الكشوفات السابقة
          </button>
          <button
            onClick={() => handleSave()}
            className="px-3 py-2 md:px-4 md:py-2 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-xl transition-colors font-bold flex items-center gap-1 md:gap-2 shadow-sm whitespace-nowrap"
          >
            <Save size={18} /> حفظ الكشف
          </button>
          <button
            onClick={handleNewSheet}
            className="px-3 py-2 md:px-4 md:py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition-colors font-bold flex items-center gap-1 md:gap-2 shadow-sm whitespace-nowrap"
          >
            <Plus size={18} /> جدول جديد
          </button>
          <button
            onClick={() => setShowCombineModal(true)}
            className="px-3 py-2 md:px-4 md:py-2 bg-amber-100 hover:bg-amber-200 text-amber-700 rounded-xl transition-colors font-bold flex items-center gap-1 md:gap-2 shadow-sm whitespace-nowrap"
          >
            <Combine size={18} /> جمع الجداول
          </button>
          <button
            onClick={exportTableToExcel}
            className="px-3 py-2 md:px-4 md:py-2 bg-green-100 hover:bg-green-200 text-green-700 rounded-xl transition-colors font-bold flex items-center gap-1 md:gap-2 shadow-sm whitespace-nowrap"
          >
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          ...
          <select
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm font-bold focus:ring-2 focus:ring-blue-500"
          >
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden overflow-x-auto relative min-h-[400px]">
        {selectedGrade && selectedSection ? (
          <table
            id="grade-sheet-table"
            className="w-full text-center text-sm border-collapse min-w-[1200px]"
          >
            <thead>
              {/* Top Headers */}
              <tr>
                <th
                  rowSpan={2}
                  className="border border-slate-200 bg-amber-500 text-white w-12 font-black"
                >
                  م
                </th>
                <th
                  rowSpan={2}
                  className="border border-slate-200 bg-amber-50 w-48 text-slate-800 font-black p-2 min-w-[150px]"
                >
                  اسم الطالب
                </th>
                ...
                {months.map(
                  (m, mIdx) =>
                    visibleMonths.includes(mIdx) && (
                      <th
                        key={mIdx}
                        colSpan={7}
                        className={`border border-slate-300 font-black text-slate-800 p-2 text-lg ${mIdx === 0 ? "bg-blue-200 bg-month-0" : mIdx === 1 ? "bg-rose-200 bg-month-1" : "bg-purple-200 bg-month-2"}`}
                      >
                        <input
                ...
                <th
                  rowSpan={2}
                  className="border border-slate-300 bg-orange-100 p-2 w-24 align-top"
                >
                  <div className="flex flex-col items-center justify-center gap-2 h-full">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        ...
                        setMaxScores({
                          ...maxScores,
                          term_result: parseFloat(e.target.value),
                        })
                      }
                ...
                <th
                  rowSpan={2}
                  className="border border-slate-300 bg-orange-200 p-2 w-24"
                >
                  اختبار فصلي
                </th>
              </tr>
                      ...
                      <React.Fragment key={"sub-" + mIdx}>
                        {monthCols.map((mc, mcIdx) => {
                          ...
                          return (
                            <th
                              key={`head-${mIdx}-${mc.id}`}
                              className={`border border-slate-300 p-2 font-bold text-xs ${mc.isTotal ? "bg-yellow-300 text-black shadow-inner shadow-yellow-500/50" : mIdx === 0 ? "bg-blue-100 bg-month-0" : mIdx === 1 ? "bg-rose-100 bg-month-1" : "bg-purple-100 bg-month-2"}`}
                            >
                              <div className="flex flex-col items-center justify-between w-full min-h-[6rem] py-1">
                                <div className="flex flex-col items-center gap-1 w-full flex-grow">
                                  <span className="whitespace-normal leading-tight text-center break-words max-w-[70px] flex-grow flex items-center">
                                    {mc.label}
                                  </span>
                                  {!mc.isTotal && (
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        if (canEditData && e.target.value) {
                                          setMaxScores({
                                            ...maxScores,
                                            [mc.id]: parseFloat(e.target.value),
                                          });
                                        }
                                      }}
                                      disabled={!canEditData}
                                ...
                                {isDisabled && (
                                  <span className="text-[10px] text-red-600 font-bold bg-white/50 px-1 rounded leading-none mt-1 shrink-0">
                                    غير معتمد
                                  </span>
                                )}
                              </div>
            ...
            <tbody>
              {displayedStudents.map((s, sIdx) => {
                ...
                const toggleRow = (e: React.MouseEvent) => {
                  ...
                  if (isRowSelected) {
                    setHighlightedRows(
                      highlightedRows.filter((id) => id !== s.id),
                    );
                  } else {
                ...
                return (
                  <tr
                    key={s.id}
                    className={`hover:bg-slate-50 transition-all ${isRowSelected ? "ring-2 ring-black shadow-lg relative z-20 bg-white" : ""}`}
                  >
                    <td className="border border-slate-200 p-1 font-bold bg-amber-50">
                      {sIdx + 1}
                    </td>
                    <td
                      className="border border-slate-200 p-2 font-bold text-right truncate max-w-[150px] bg-slate-50 cursor-pointer hover:bg-amber-100 transition-colors"
                      title={s.name}
                      onClick={toggleRow}
                    >
                      {s.name}
                        ...
                        visibleMonths.includes(mIdx) && (
                          <React.Fragment key={"r-" + s.id + "-" + mIdx}>
                            {monthCols.map((mc, mcIdx) => {
                              ...
                              const isColSelected =
                                selectedColIndex === cIndex && !isRowSelected;
                              const isDisabled =
                                disabledCols.includes(colKeyStr);

                              const maxVal = maxScores[mc.id];
                              const isFailingField =
                                !mc.isTotal &&
                                !isNaN(numVal) &&
                                mc.id !== "ratio_m" &&
                                maxVal &&
                    ...
                    {finalCols.map((fc, fcIdx) => {
                      ...
                      const maxVal = isTermResult
                        ? maxScores.term_result || 20
                        : maxScores.term_exam || 0;
                      const isFailing =
                        !isNaN(valNum) && maxVal > 0 && valNum < maxVal / 2;
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            value={val}
                            onChange={(e) =>
                              handleCellChange(s.id, colKeyStr, e.target.value)
                            }
                          />
                        </td>
                      );
              ...
              {displayedStudents.length > 0 && (
                <tr className="bg-slate-100 font-black">
                  <td
                    colSpan={2}
                    className="border border-slate-300 p-2 text-left bg-slate-200"
                  >
                    الإجمالي / النسبة
                  </td>
                  {months.map(
                        ...
                        <React.Fragment key={"tot-" + mIdx}>
                          {monthCols.map((mc, mcIdx) => {
                            ...
                            return (
                              <td
                                key={colKeyStr}
                                className="border border-slate-300 p-1 text-xs text-center relative pointer-events-none text-slate-700"
                              >
                                <div className="text-blue-700 border-b border-slate-300/50 mb-1 pb-1">
                  ...
                  {finalCols.map((fc, fcIdx) => {
                    ...
                    return (
                      <td
                        key={colKeyStr}
                        className="border border-slate-300 p-1 text-xs text-center relative pointer-events-none text-slate-700 bg-orange-100/50"
                      >
                        <div className="text-orange-700 border-b border-orange-200 mb-1 pb-1">
                          {Number(total) > 0 ? total : ""}
                        </div>
          ...
          <div className="flex flex-col items-center justify-center p-20 text-slate-400">
            ...
            <p className="font-bold text-lg mb-2">
              يرجى اختيار الصف والشعبة لعرض أسماء الطلاب
            </p>
      ...
      {selectedGrade && selectedSection && (
        <div className="mt-8 px-8 flex justify-between items-center font-bold text-slate-800 pb-10 max-w-[1200px] mx-auto">
          <div className="text-center w-48">
            ...
            <p className="mt-4 text-blue-800 text-xl border-b-2 border-dashed border-slate-300 pb-1 inline-block min-w-[200px]">
              {currentUser?.name || "........................"}
            </p>
          </div>
          <div className="text-center w-48">
            <p className="text-lg">مدير المدرسة</p>
            <p className="mt-4 border-b-2 border-dashed border-slate-300 pb-1 inline-block min-w-[200px] h-8 text-xl text-slate-400">
              ........................
            </p>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm flex flex-col shadow-2xl relative">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              ...
              <button
                onClick={() => setShowVisibleMonthsModal(false)}
            ...
            <div className="p-6 flex flex-col gap-4">
              {months.map((m, mIdx) => (
                ...
                >
                  <input
                    ...
                    onChange={() => {
                      let newVm = visibleMonths.includes(mIdx)
                        ? visibleMonths.filter((v) => v !== mIdx)
                        : [...visibleMonths, mIdx];
              ...
              <button
                onClick={() => setShowVisibleMonthsModal(false)}
                className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors"
              >
                تطبيق وإغلاق
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg flex flex-col shadow-2xl relative my-auto">
            ...
            <div className="p-6 flex flex-col gap-6">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setIndicatorConfig((prev) => ({
                          ...prev,
                          months: prev.months.includes(mIdx)
                            ? prev.months.filter((v) => v !== mIdx)
                            : [...prev.months, mIdx],
                        }));
                      }}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {monthCols
                    .filter((mc) => !mc.isTotal && mc.id !== "ratio_m")
                    .map((mc) => (
                      <button
                        key={mc.id}
                        onClick={() => {
                          setIndicatorConfig((prev) => ({
                            ...prev,
                            fields: prev.fields.includes(mc.id)
                              ? prev.fields.filter((v) => v !== mc.id)
                              : [...prev.fields, mc.id],
                          }));
                        }}
              ...
              <button
                onClick={() => setShowIndicatorModal(false)}
                className="mt-2 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors"
              >
                تطبيق
              </button>
            </div>
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-2">
              {sheets.length === 0 ? (
                <div className="text-center p-8 text-slate-400 font-bold">
                  لا يوجد كشوفات سابقة محفوظة
                </div>
              ) : (
                sheets.map((sh) => (
                  <div
                    key={sh.id}
                    className="p-4 border border-slate-100 rounded-2xl flex items-center justify-between hover:bg-slate-50 group cursor-pointer"
                    onClick={() => loadSheet(sh)}
                  >
                    <div>
                      <p className="font-black text-slate-800 mb-1">
                        {sh.sheetName || "بدون اسم"}
                      </p>
                      <p className="text-xs font-bold text-slate-500">
                        {sh.date} - {sh.grade} ({sh.section})
                      </p>
                    </div>
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={(e) => handleDeleteSheet(sh.id!, e)}
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl relative">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-3">
              <p className="text-sm font-bold text-slate-500 mb-4">
                اختر الكشوفات التي ترغب في جمع درجاتها (سيتم دمج القيم للكشوفات
                المختارة)
              </p>
              {sheets.length === 0 ? (
                <div className="text-center p-8 text-slate-400 font-bold">
                  لا يوجد كشوفات محفوظة
                </div>
              ) : (
                sheets.map((sh) => (
                  <label
                    key={sh.id}
                    className="flex items-center gap-3 p-4 border border-slate-100 rounded-2xl hover:bg-slate-50 cursor-pointer"
                  >
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) => {
                        if (e.target.checked)
                          setSelectedCombineSheets([
                            ...selectedCombineSheets,
                            sh.id!,
                          ]);
                        else
                          setSelectedCombineSheets(
                            selectedCombineSheets.filter((id) => id !== sh.id!),
                          );
                      }}
                    />
                    <div>
            ...
            <div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50 rounded-b-3xl">
              ...
              <button
                ...
                className="px-6 py-3 font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-200 transition-colors"
              >
                تأكيد لجمع الجداول
              </button>
            </div>
