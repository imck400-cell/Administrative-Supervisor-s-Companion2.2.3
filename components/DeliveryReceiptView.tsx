...
export const DeliveryReceiptView: React.FC<Props> = ({ onClose }) => {
  ...
  const [applyStatus, setApplyStatus] = useState<
    "delivered" | "in_progress" | "not_delivered"
  >("in_progress");
  ...
  const supervisorJob =
    ...
    (currentUser?.role === "admin" ? "مدير نظام" : "مشرف إداري");
  const school =
  ...
  const academicYear =
    currentUser?.selectedYear || data.profile?.year || "1445-1446";
  const grade =
    currentUser?.grades && currentUser.grades.length > 0
      ? currentUser.grades.join(", ")
      : "الجميع";
  const section =
    currentUser?.sections && currentUser.sections.length > 0
      ? currentUser.sections.join(", ")
      : "الجميع";
    ...
    if (!isAdmin) {
      if (currentUser?.selectedSchool) {
        ...
        staff = staff.filter((s: any) =>
          userSchools.includes(String(s.school || "").trim()),
        );
      }
      if (currentUser?.selectedBranch) {
        staff = staff.filter(
          (s: any) => s.branch === currentUser.selectedBranch,
        );
      }
      if (currentUser?.grades && currentUser.grades.length > 0) {
        staff = staff.filter(
          (s: any) =>
            !s.grade || currentUser.grades!.includes(String(s.grade).trim()),
        );
      }
    }
    return staff;
  ...
  const uniqueBranches = Array.from(
    new Set(availableStaff.map((s: any) => s.branch).filter(Boolean)),
  ) as string[];
  const uniqueGrades = Array.from(
    new Set(availableStaff.map((s: any) => s.grade).filter(Boolean)),
  ) as string[];
  const uniqueSections = Array.from(
    new Set(availableStaff.map((s: any) => s.section).filter(Boolean)),
  ) as string[];
  ...
  const saveWork = (
    currentItems: DeliveryReceiptItem[] = items,
  ...
  const applyToSelected = (
    field: keyof DeliveryReceiptItem,
    value: any,
    condition?: (item: DeliveryReceiptItem) => boolean,
  ) => {
    const newItems = items.map((t) => {
      ...
      if (applySections.length > 0 && !applySections.includes(t.section))
        return t;
  ...
  const handleApplyFilter = () => {
    let results: (DeliveryReceiptItem & {
      _parent: DeliveryReceiptReport;
      uniqueId: string;
    })[] = [];
    existingReports.forEach((report) => {
      if (
        report.dateStr >= filterStartDate &&
        report.dateStr <= filterEndDate
      ) {
        if (
          filterSelectedReports.length === 0 ||
          filterSelectedReports.includes(report.id)
        ) {
          report.items.forEach((t) => {
            const matchesStatus =
              filterSelectedStatus.length === 0 ||
              filterSelectedStatus.includes(t.status as string);
            const matchesAction =
              filterSelectedActions.length === 0 ||
              filterSelectedActions.includes(t.actionTaken as string);

            if (matchesStatus && matchesAction) {
              results.push({
                ...t,
                _parent: report,
                uniqueId: `${report.id}-${t.id}`,
              });
            }
          });
        }
    ...
    results.sort((a, b) => {
      if (a._parent.dateStr !== b._parent.dateStr)
        return b._parent.dateStr.localeCompare(a._parent.dateStr);
      return 0;
    });
  ...
  const generateWhatsAppMessage = (
    tasksToExport: (DeliveryReceiptItem & { _parent: DeliveryReceiptReport })[],
  ) => {
    ...
    tasksToExport.forEach((t, idx) => {
      ...
      if (t.deliveryDateStr)
        msg += `📤 *تاريخ التسليم:* ${t.deliveryDateStr}\n`;
    ...
    const itemsToSend =
      selectedTaskIds.length > 0
        ? filteredResults.filter((t) =>
            selectedTaskIds.includes((t as Record<string, any>).uniqueId),
          )
        : filteredResults;
  ...
  const handleToggleAllSelection = () => {
    ...
    if (selectedTaskIds.length === filteredResults.length) {
      ...
      setSelectedTaskIds(
        filteredResults.map((t) => (t as Record<string, any>).uniqueId),
      );
    }
  };
      ...
      <div className="space-y-6 font-arabic p-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button
              onClick={handleExportWhatsAppAll}
          ...
          <table className="w-full text-right whitespace-nowrap">
            <thead>
              <tr className="bg-gradient-to-l from-blue-600 to-blue-800 text-white">
                <th className="p-4 border-b border-blue-900 w-12 text-center">
                  <input
                    ...
                    checked={
                      ...
                      selectedTaskIds.length === filteredResults.length
                    }
                ...
                <th className="p-4 border-b border-blue-900">
                  الاستلام والتسليم
                </th>
                ...
                <th className="p-4 border-b border-blue-900 text-center w-16">
              ...
              {filteredResults.map((t, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-4 border-e border-slate-100 text-center">
                      {t.formCount}
                    </td>
                    <td className="p-4 border-e border-slate-100">
                      <div className="text-sm">
                        <span className="text-slate-500">استلام:</span>{" "}
                        {t.receiveDateStr}
                      </div>
                      <div className="text-sm">
                        <span className="text-slate-500">تسليم:</span>{" "}
                        {t.deliveryDateStr}
                      </div>
                    </td>
                    <td className="p-4 border-e border-slate-100">
                    ...
                    <td className="p-4 whitespace-normal min-w-[150px] border-e border-slate-100">
                      {t.notes}
                    </td>
      ...
      <div className="space-y-6 font-arabic p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {existingReports
            .filter(
              (t) =>
                t.userId === currentUser?.id || currentUser?.role === "admin",
            )
            .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
            .map((report) => (
              <div
                key={report.id}
                className="bg-white border-2 border-slate-100 rounded-xl p-4 shadow-sm hover:border-blue-200 transition-colors"
              >
                ...
                <p className="text-slate-600 font-medium mb-1 truncate">
                  {report.supervisorName}
                </p>
                <p className="text-sm text-slate-500 mb-4 truncate">
                  {report.school} - {report.branch}
                </p>
                <button
                  onClick={() => {
  ...
  return (
    <div className="space-y-6 font-arabic max-w-7xl mx-auto p-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-wrap items-center gap-3 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        <button
          onClick={() => saveWork(items)}
          className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 hover:shadow-xl hover:-translate-y-0.5 transition-all"
        >
          <Save size={20} />
          حفظ العمل (تلقائي)
        </button>
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-orange-50 text-orange-600 border border-orange-200 rounded-xl font-bold hover:bg-orange-100 transition-colors"
        >
          <Archive size={20} />
          الأرشيف
        </button>
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-xl font-bold hover:bg-emerald-100 transition-colors"
        >
          <Filter size={20} />
          مؤشرات كشف الاستلام والتسليم
        </button>
        {onClose && (
          <button
            onClick={onClose}
            className="mr-auto btn-secondary bg-slate-100"
          >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm text-slate-600 block mb-1 font-bold">
                  اسم الكشف الجديد
                </label>
                <input
                  type="text"
                  ...
                  placeholder="مثال: كشف استلام تحضير الدروس"
                />
              </div>
              <div>
                <label className="text-sm text-slate-600 block mb-1 font-bold">
                  ...
                  <div className="max-h-24 overflow-y-auto bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-1">
                    {uniqueGrades.map((g) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={applyGrades.includes(g)}
                          onChange={(e) => {
                            if (e.target.checked)
                              setApplyGrades([...applyGrades, g]);
                            else
                              setApplyGrades(
                                applyGrades.filter((x) => x !== g),
                              );
                          }}
                  ...
                  <div className="max-h-24 overflow-y-auto bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-1">
                    {uniqueSections.map((s) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={applySections.includes(s)}
                          onChange={(e) => {
                            if (e.target.checked)
                              setApplySections([...applySections, s]);
                            else
                              setApplySections(
                                applySections.filter((x) => x !== s),
                              );
                          }}
                          className="w-4 h-4 rounded text-blue-600"
                        />
              ...
              <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      type="number"
                      value={applyFormCount}
                      onChange={(e) => setApplyFormCount(e.target.value)}
                      className="input-field"
                      placeholder="10"
                    />
                  </div>
                  <button
                    onClick={() => applyToSelected("formCount", applyFormCount)}
                ...
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      type="date"
                      value={applyReceiveDate}
                      onChange={(e) => setApplyReceiveDate(e.target.value)}
                      className="input-field"
                    />
                  </div>
                  <button
                    onClick={() =>
                      applyToSelected("receiveDateStr", applyReceiveDate)
                    }
                    className="btn-secondary h-[42px] px-3"
                  >
                    تطبيق
                ...
                <div className="flex items-end gap-2">
                  <div className="flex-1">
                    ...
                    <input
                      type="date"
                      value={applyDeliveryDate}
                      onChange={(e) => setApplyDeliveryDate(e.target.value)}
                      className="input-field"
                    />
                  </div>
                  <button
                    onClick={() =>
                      applyToSelected("deliveryDateStr", applyDeliveryDate)
                    }
                    className="btn-secondary h-[42px] px-3"
                  >
                    تطبيق
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => applyToSelected("status", applyStatus)}
                      className="btn-secondary h-[42px] px-3"
                    >
                      تطبيق
                    </button>
                ...
                <div className="md:col-span-2 grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-bold text-slate-700 block">
                      الإجراء المتخذ (لمن لم يسلم)
                    </label>
                    <div className="flex items-center gap-2">
                      <select
                        value={applyAction}
                        onChange={(e) => setApplyAction(e.target.value)}
                        className="input-field flex-1"
                      >
                        ...
                        <option value="الرفع إلى الإدارة العليا">
                          الرفع إلى الإدارة العليا
                        </option>
                      </select>
                      <button
                        onClick={() =>
                        ...
                        className="btn-secondary h-[42px] px-3 whitespace-nowrap text-xs"
                      >
                        تطبيق على (لم يسلم)
                      </button>
                    </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <button
                        onClick={() => applyToSelected("notes", applyNotes)}
                        className="btn-secondary h-[42px] px-3"
                      >
                        تعميم على الجميع
                      </button>
            ...
            <table className="w-full text-right text-sm">
              <thead>
                <tr className="bg-slate-50 text-slate-700 border-b border-slate-200">
                  ...
                  <th className="p-3 font-bold min-w-[120px]">
                    المدرسة والفرع
                  </th>
                  ...
                  <th className="p-3 font-bold min-w-[140px]">
                    الإجراء المتخذ
                  </th>
                ...
                {items.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-center">
                      <input
                        type="text"
                        value={item.formCount}
                        onChange={(e) =>
                          updateItem(item.id, { formCount: e.target.value })
                        }
                        className="w-full text-center border border-slate-200 rounded p-1"
                      />
                    </td>
                    <td className="p-3 text-center">
                      <input
                        type="date"
                        value={item.receiveDateStr}
                        onChange={(e) =>
                          updateItem(item.id, {
                            receiveDateStr: e.target.value,
                          })
                        }
                    ...
                    <td className="p-3 text-center">
                      <input
                        type="date"
                        value={item.deliveryDateStr}
                        onChange={(e) =>
                          updateItem(item.id, {
                            deliveryDateStr: e.target.value,
                          })
                        }
                        className="w-full text-sm border border-slate-200 rounded p-1"
                      />
                    ...
                    <td className="p-3">
                      <select
                        value={item.status || ""}
                        onChange={(e) =>
                          updateItem(item.id, { status: e.target.value as any })
                        }
                        className={`w-full text-sm border border-slate-200 rounded p-1.5 focus:ring-1 outline-none ${
                    ...
                    <td className="p-3">
                      <select
                        ...
                        onChange={(e) =>
                          updateItem(item.id, {
                            actionTaken: e.target.value as any,
                          })
                        }
                        className="w-full text-sm border border-slate-200 rounded p-1.5 focus:ring-1 outline-none text-slate-700"
                      >
                        <option value="">بدون إجراء</option>
                        <option value="تنبيه شفوي">تنبيه شفوي</option>
                        <option value="تعهد خطي">تعهد خطي</option>
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan={11} className="p-8 text-center text-slate-500">
                      لم يتم العثور على معلمين متاحين. قد تحتاج إلى إضافة موظفين
                      في შؤون الموظفين.
                    </td>
                  </tr>
                )}
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-slate-600 block mb-2 font-bold">
                    من تاريخ
                  </label>
                  <input
                    type="date"
                ...
                <div className="max-h-32 overflow-y-auto bg-slate-50 border border-slate-200 rounded-lg p-2 space-y-1">
                  {existingReports.map((r) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={filterSelectedReports.includes(r.id)}
                        onChange={(e) => {
                          if (e.target.checked)
                            setFilterSelectedReports([
                              ...filterSelectedReports,
                              r.id,
                            ]);
                          else
                            setFilterSelectedReports(
                              filterSelectedReports.filter((id) => id !== r.id),
                            );
                        }}
                        className="w-4 h-4 rounded text-blue-600"
                      />
                      <span className="text-sm">
                        {r.reportName} - {r.dateStr}
                      </span>
                    </label>
                  ))}
                  {existingReports.length === 0 && (
                    <div className="text-sm text-slate-500">لا توجد كشوفات</div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={filterSelectedStatus.includes("delivered")}
                      onChange={(e) => {
                        if (e.target.checked)
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={filterSelectedStatus.includes("in_progress")}
                      onChange={(e) => {
                        if (e.target.checked)
                          setFilterSelectedStatus([
                            ...filterSelectedStatus,
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={filterSelectedStatus.includes("not_delivered")}
                      onChange={(e) => {
                        if (e.target.checked)
                          setFilterSelectedStatus([
                            ...filterSelectedStatus,
                ...
                <div className="flex flex-wrap gap-3">
                  {[
                    ...
                    <label key={act} className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={filterSelectedActions.includes(act)}
                        onChange={(e) => {
                          if (e.target.checked)
                            setFilterSelectedActions([
                              ...filterSelectedActions,
                              act,
                            ]);
                          else
                            setFilterSelectedActions(
                              filterSelectedActions.filter((x) => x !== act),
                            );
                        }}
                        className="w-4 h-4 rounded text-blue-600"
                      />
