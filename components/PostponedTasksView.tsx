...
export const PostponedTasksView: React.FC<Props> = ({ onClose }) => {
  ...
  const [filteredResults, setFilteredResults] = useState<
    (PostponedTaskItem & { _parent: PostponedTask; uniqueId: string })[] | null
  >(null);
  ...
  const saveWork = (
    currentTasks: PostponedTaskItem[] = tasks,
  ...
  const updateTask = (id: string, updates: Partial<PostponedTaskItem>) => {
    setTasks((prev) => {
      ...
      if (i >= 0) {
        ...
        if (
          i === newTasks.length - 1 &&
          updates.taskText !== undefined &&
          updates.taskText.trim() !== ""
        ) {
          newTasks.push({
            id: crypto.randomUUID(),
            taskText: "",
  ...
  const handleApplyFilter = () => {
    let results: (PostponedTaskItem & {
      _parent: PostponedTask;
      uniqueId: string;
    })[] = [];
  ...
  const generateWhatsAppMessage = (
    tasksToExport: (PostponedTaskItem & { _parent: PostponedTask })[],
  ) => {
              ...
              {filteredResults.map((t, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-4 border-e border-slate-100">
                      {t._parent.supervisorName} ({t._parent.supervisorJob})
                    </td>
                    <td className="p-4 border-e border-slate-100">
                      {t._parent.school} - {t._parent.branch}
                    </td>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <textarea
                  value={task.taskText}
                  onChange={(e) =>
                    updateTask(task.id, { taskText: e.target.value })
                  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <button
                      onClick={() =>
                        updateTask(task.id, { status: "not_done" })
                      }
                      className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all ${
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={filterStatus.not_done}
                      onChange={(e) =>
                        setFilterStatus((prev) => ({
                          ...prev,
                          not_done: e.target.checked,
                        }))
                      }
                      className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 cursor-pointer border-slate-300"
                    />
                    <span className="font-bold text-rose-800">
                      المهام التي لم تنجز (لم ينفذ)
                    </span>
                  </label>
                </div>
              </div>
