...
export const GlobalProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateData = (newData: Partial<AppData>, overrideSchools?: string[]) => {
    if (isAuthenticated && currentUser) {
      ...
      const strictlySharedKeys = ['profile', 'users', 'availableSchools', 'availableYears', 'secretariatStudents', 'secretariatStaff', 'selfEvaluationTemplates', 'metricsList', 'adminMetricsList', 'branchMetrics', 'adminBranchMetrics', 'adminActivitiesList', 'adminBranchActivities', 'adminIndividualReportFields'];
      const customizableKeys = ['taskTemplates', 'customViolationElements', 'absenceManualAdditions', 'absenceExclusions'];
      ...
      for (const key of Object.keys(newData) as Array<keyof AppData>) {
        if (strictlySharedKeys.includes(key)) {
          ...
          else if ((key === 'adminMetricsList' || key === 'adminBranchMetrics' || key === 'adminActivitiesList' || key === 'adminBranchActivities' || key === 'adminIndividualReportFields') && (isSecretariatEnabled || canEditAdminFollowUp)) canSave = true;
